...
namespace TqkLibrary.SeleniumSupport
{
  public static class ProxyLoginExtension
  {
    ...
    const string background_js = "var config={mode:\"fixed_servers\",rules:{singleProxy:{scheme:\"http\",host:\"{host}\",port:{port}},bypassList:[\"localhost\"]}};chrome.proxy.settings.set({value:config,scope:\"regular\"},function(){});functioncallbackFn(details){return{authCredentials:{username:\"{username}\",password:\"{password}\"}};}chrome.webRequest.onAuthRequired.addListener(callbackFn,{urls:[\"<all_urls>\"]},['blocking']);";

    public static void GenerateExtension(string filepath, string host,int port,string username, string password)
    {
      string background = background_js.Replace("{host}", host).Replace("{port}", port.ToString()).Replace("{username}", username).Replace("{password}", password);
      ZipFile zipFile = ZipFile.Create(filepath);
      zipFile.BeginUpdate();
      using(CustomStaticDataSource staticDataSource = new CustomStaticDataSource(manifest_json)) zipFile.Add(staticDataSource, "manifest.json");
      using (CustomStaticDataSource staticDataSource = new CustomStaticDataSource(background)) zipFile.Add(staticDataSource, "background.js");
